...
def get_csv_file_path(received_device_name):
    ...
    return os.path.join(data_dir, f"{received_device_name}_{current_date}.csv")
...
def receive_data():
    try:
        ...
        csv_file_path = get_csv_file_path(received_device_name)
...
def get_data():
    try:
        received_device_name = request.args.get('device_name')
        print(f"get dev name: {received_device_name}") 
        csv_file_path = get_csv_file_path(received_device_name)
        data_df = pd.read_csv(csv_file_path)
